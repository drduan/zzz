...
<c:if test="${rowCount == 0}">
	<c:if test="${!empty(useFlag)}">
		<div
			style="border: 1px solid #666; padding: 2 5 2 5; background: #efefef; color: #333">没有相关记录</div>
	</c:if>
	<c:if test="${empty(useFlag)}">
	...
	<table style="align-content: center">
		<tr>
			<td><c:if test="${(pageCount <= 11)}">
				...
				</c:if> <c:if test="${(pageCount > 11)&&(pageNum + 5 > pageCount)}">
					<c:set var="startPage" value="${pageCount - 10 }" />
					<c:set var="endPage" value="${pageCount}" />
				</c:if> <c:if
					test="${(pageCount > 11)&&(pageNum + 5 <= pageCount)&&(pageNum - 5 < 1)}">
					<c:set var="startPage" value="1" />
					...
					<ul class="pagination">
						...
						<c:if test="${(pageCount > 1)}">
							<c:if test="${(pageNum != 1)}">
								...
								<li><a href="${newPageUrl}" aria-label="Previous"> <span
							...
							<c:if test="${(pageNum == 1)}">
								<li class="disabled"><span><span
										class="glyphicon glyphicon-fast-backward" aria-hidden="true"></span></span></li>
								<li class="disabled"><span><span
										class="glyphicon glyphicon-backward" aria-hidden="true"></span></span></li>
							</c:if>
							...
							<c:if test="${(pageCount != pageNum)}">
								...
								<c:if test="${(pageCount > 11)}">
									...
									<c:if test="${!fn:endsWith(pageUrl,'?')}">
										<c:set var="newPageUrl"
											value="${pageUrl}&pageNum=${pageCount}" />
									</c:if>
								...
								<li><a href="${newPageUrl}" aria-label="Next"> <span
