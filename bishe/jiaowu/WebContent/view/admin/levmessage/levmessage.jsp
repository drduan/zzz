...
$(function(){
		$('body').on('click','button[name="readLe"]',function(){
			$.post("${pageContext.request.contextPath}/admin/user/readLevMG",{"levID":this.id},function(result){
				if(result.flag){
					initData(result.list);
				}
			})
		})
		$('body').on('click','button[name="delLe"]',function(){
			$.post("${pageContext.request.contextPath}/admin/user/delLevMG",{"levID":this.id},function(result){
				if(result.flag){
					initData(result.list);
				}
			})
		...
		function initData(list){
			...
			for(var i in list){
				...
				var tr="<tr><td>"+(i+1)+
				"</td><td>"+list[i].s_id+
				"</td><td>"+list[i].levName+
				"</td><td>"+list[i].levEmail+
				"</td><td>"+list[i].levCon+
				"</td><td>"+list[i].levTime+
				"</td><td>"+statu+
				"</td><td>"+options+"</td></tr>"
				$("#levMgTB").append(tr);
			}
		...
		<table class="table">
			...
			<tbody id="levMgTB">
				<c:if test="${_LEVMESSAGE_!=null}">
					<c:forEach items="${_LEVMESSAGE_}" var="i" varStatus="ii">
						<tr>
							<th>${ii.count}</th>
							<td>${i.s_id}</td>
							<td>${i.levName}</td>
							...
							<td>
									<c:if test="${i.levFlag==0}">
										<button class="btn btn-default btn-xs" id="${i.levID}" name="readLe">标记已阅</button>
									</c:if>
									<button class="btn btn-default btn-xs" id="${i.levID}" name="delLe">删除</button>
