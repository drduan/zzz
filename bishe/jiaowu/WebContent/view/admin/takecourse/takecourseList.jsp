									...
									<div class="panel-body">
										<ul class="nav nav-pills">
											...
											<li role="presentation"><a
												...
												>课程管理</a></li>
												<li role="presentation"><a
												href="${pageContext.request.contextPath}/admin/takecourse/getAdminTakeCourse" class="btn btn-info">选课课程管理</a></li>
										...
										<div class="col-md-12">
											<div class="panel panel-info">
												...
												<div class="panel-body">
													<c:if test="${!empty takecourse.list }">
														<div class="table-responsive ">
															<form name="courseForm" method="post"
																action="${pageContext.request.contextPath}/admin/takecourse/delTakeCourseByIds"
																onsubmit="return check()">
																<button class="btn btn-info" type="submit">批量删除</button>
																&nbsp;&nbsp;<a
																	href="${pageContext.request.contextPath}/admin/takecourse/handleTakeCourse"
																	type="button" class="btn btn-info">添加选课课程</a>
																<table class="table table-striped">
																	<thead>
																		<tr>
																			<th><input type="checkbox" id="selAllTakeCourse"
																				onchange="selTakeCourseBut('takecourseForm', 'takecourseIds')" />
																				全选</th>
																			<th>#</th>
																			<th>选课课程名</th>
																	...
																	<tbody>
																		<c:forEach items="${takecourse.list}" var="g"
																			varStatus="vs">
																			<tr>
																				<td><input type="checkbox" name="takecourseIds"
																					value="${g.takecourseId }" /></td>
																				<td>${vs.count }</td>
																				<td>${g.takecourseName }</td>
																				<td>${g.takecourseClass }</td>
																				...
																				<td><a
																					href="${pageContext.request.contextPath}/admin/takecourse/handleTakeCourse?takecourseId=${g.takecourseId}">修改</a>
																					| <a
																					href="${pageContext.request.contextPath}/admin/takecourse/delTakeCourse?takecourseId=${g.takecourseId}"
																					onclick="return confirm('确定要删除这个选课吗？')">删除</a></td>
													...
													<c:if test="${empty takecourse.list }">选课课程信息为空</c:if>
												</div>
											</div>
